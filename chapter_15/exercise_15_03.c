...
displayStatInfo(const struct stat *sb)
{
    ...
    char buf[100];
    strftime(buf, sizeof(buf), "%D %T", gmtime(&sb->st_atim.tv_sec));
    printf("Last file access:         %s.%09ld UTC\n", buf, sb->st_atim.tv_nsec);
    strftime(buf, sizeof(buf), "%D %T", gmtime(&sb->st_mtim.tv_sec));
    printf("Last file modification:   %s.%09ld UTC\n", buf, sb->st_mtim.tv_nsec);
    strftime(buf, sizeof(buf), "%D %T", gmtime(&sb->st_ctim.tv_sec));
    printf("Last status change:       %s.%09ld UTC\n", buf, sb->st_ctim.tv_nsec);
}
